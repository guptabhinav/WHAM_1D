...
program fes_calc
implicit none
integer i, iter, ncv, umbr_n, nbin1
...
if (parent) then
   if (ncv.eq.1) then
     ...
     print *, 'bins=', nbin1
   else if (ncv.ge.2)then
      STOP '2 or more CVs not implemented'
   end if
end if
...
allocate(biased_prob(nbin1,umbr_n))
allocate(prob(nbin1))
...
!
      ...
      do i_s1=1,nbin1 !US
         read(3,*)dummy,biased_prob(i_s1,i_umbr)!,dummy
      end do
   enddo
end if
...
call RBcast(biased_prob,nbin1*umbr_n)

!PERFORMS WHAM.
if (parent)  write(*,*) 'wham begins'

!distribute the first grid data over the processors
CALL DistributeGrids(ncv,grid0,grid,rank,gleng1_min,gleng1_max)

write(*,*)'new_grid', gleng1_min, gleng1_max,rank, grid(1,1)
...
scf_loop : do
       iter=iter+1
!
       call wham_scf(a,umbr_k,umbr_mean,nmax,grid0,biased_prob,umbr_n,nbin1,ncv,kt,prob,cnvg,gleng1_min,gleng1_max)
!
       if (parent) write(*,*) 'iteration #', iter, 'convergence =',cnvg

       if (mod(iter,100) .eq. 0 ) then
           call print_pmf(ncv,nbin1,grid0,kt,prob,parent)
       endif

       if(cnvg.lt.toler)then
          if (parent) write(*,*)'** convergence achieved **'
          exit scf_loop
       else if(iter.gt.999999)then
          if(parent) write(*,*)'** !ERROR! WHAM did not converge!'
          exit scf_loop
       end if
end do scf_loop
...
!***********************************************!
 ...
 subroutine print_pmf(ncv,nbin1,grid0,kt,prob,parent)
!prints pmf 
 implicit none
 integer:: i_s1, i_s2
 integer:: nbin1,  ncv
 real*8 :: prob(nbin1), grid0(3,ncv)
 real*8 :: s1, s2, kt, dum
 logical:: parent
 real*8, allocatable :: prob1(:)
 character (len=50):: f2
 
 allocate(prob1(nbin1))
 prob1=0.0
 CALL GlobSumR(prob,prob1,nbin1)
 if (parent)then
 f2= 'free_energy'
 open( unit =7 , file = f2, status =  'unknown' )
 do i_s1=1,nbin1 !US cv
     s1=DFLOAT(i_s1-1)*grid0(3,1)+grid0(1,1)
     dum= -kt*DLOG(prob1(i_s1))
     write(7,'(3E16.8)') s1, dum, prob(i_s1)
 enddo
...
!WHAM
subroutine wham_scf(a,umbr_k,umbr_mean,nmax,grid0,biased_prob,umbr_n,nbin1,ncv,kt,prob,cnvg,gleng1_min,gleng1_max)
implicit none
integer:: nbin1, umbr_n, ncv
integer:: nmax(umbr_n) 
real*8 :: umbr_k(umbr_n), umbr_mean(umbr_n)
!
real*8 :: num, den, dummy_v, dummy_s1, avg, del_s1, kt, dummy, cnvg
real*8 :: prob(nbin1),a(umbr_n),dum
real*8 :: grid0(3,ncv), biased_prob(nbin1,umbr_n),grid(3,ncv)
integer:: rank, gleng1_min,gleng1_max,gleng2,ngrid
!
...
do i_s1 =gleng1_min,gleng1_max !over US cv
   ...
   do i_umbr=1,umbr_n
      ...
      num=num+dfloat(nmax(i_umbr))*biased_prob(i_s1,i_umbr)
      den=den+dfloat(nmax(i_umbr))*a(i_umbr)*dummy_v
   enddo
!
   prob(i_s1)=num/den
!
   if(prob(i_s1).ne.prob(i_s1)) prob(i_s1)=1.0D-16 !remove NaN
!
   if(prob(i_s1)+1.eq.prob(i_s1)) prob(i_s1)=1.0D-16 !remove infinity
!calculate a.
   dum=grid0(3,1)
   do i_umbr=1,umbr_n
      del_s1=dummy_s1 - umbr_mean(i_umbr)
      ...
      dummy_v=dexp(-0.50d0*umbr_k(i_umbr)*del_s1*del_s1/kt)
      dummy_a(i_umbr)=dummy_a(i_umbr)+dum*dummy_v*prob(i_s1)
   end do
end do !end of US cv loop
